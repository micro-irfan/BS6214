...
high_corr <- "CYP2D6_bestNegativeCorrelation.txt"
...
write.csv(cluster_summary, "clusterProfiler_LIVER.neg.csv")
fit1 <- dotplot(GO_results, showCategory=15)
png("GO_dot_plot_LIVER.neg.png", res = 250, width = 1400, height = 1800)
print(fit1)
dev.off()

fit2 <- emapplot(GO_results, showCategory = 50)
png("GO_emap_plot_LIVER.neg.png", res = 250, width = 1400, height = 4000)
print(fit2)
dev.off()

fit <- plot(barplot(GO_results, showCategory = 15))
png("GO_plot_LIVER.neg.png", res = 250, width = 1400, height = 1800)
print(fit)
dev.off()
